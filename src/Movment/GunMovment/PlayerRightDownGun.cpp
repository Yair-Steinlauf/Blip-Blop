...
void PlayerRightDownGun::enter()
{
	const sf::IntRect& frame =
		GameAnimations::getInstance()
		.getFrame(AnimationSet::Blip, Direction::DownRight, 0);

	m_moveComponent.getEntity().setTextureRect(frame, PLAYER_FIXTURE_WIDTH, PLAYER_FIXTURE_HEIGHT);
}


std::unique_ptr<GunState> PlayerRightDownGun::move(sf::Vector2f mousePos, sf::Vector2f entityPos)
{
