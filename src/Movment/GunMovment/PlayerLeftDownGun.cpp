...
void PlayerLeftDownGun::enter()
{
	std::cout << ">> PlayerRightGun::enter()\n";

	const sf::IntRect& frame =
		GameAnimations::getInstance()
		.getFrame(AnimationSet::Blip, Direction::DownLeft, 0);

	m_moveComponent.getEntity().setTextureRect(frame);
}


std::unique_ptr<GunState> PlayerLeftDownGun::move(sf::Vector2f mousePos, sf::Vector2f entityPos)
{
